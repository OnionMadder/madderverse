...
- **Cache-bust:** `index.html` loads `game.js?v=N` + `style.css?v=N` — **bump `N` on every web change** (currently **v9**). Stale-cache "my change did nothing" reports = hard-refresh (Ctrl+Shift+R) and confirm the new `?v=`.
- **Art direction (in progress):** moving from CSS/SVG placeholder flowers to a **torn-paper collage** look. Raw paper shapes live in `florigami/shapes/`, rawpixel patterns in `florigami/patterns/`; a **petal-formation editor** at `florigami/tools/petal-editor.html` lets Onion design flower layouts → export formation JSON. A Python compositor (not built yet) will read formations + shapes + patterns to stamp the sprite sheets (`florigami/ASSETS.md` + `PAPER_ART_PROMPTS.md` document the pipeline). Recipe: patterned petals, watercolor pigment-pool shading, plain centers, code-driven radial orientation.
