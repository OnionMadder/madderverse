...
**Cache-busting.** `index.html` loads `main.js?v=N` + `style.css?v=N`; **bump `N` on every web change** (currently **v140** — see the "Big feature + value run" note at the very bottom for everything since v98; **v136 is the deployed/live web + Play vc13, v137–140 are the held next batch**). The recurring "my change reverted / does nothing" reports were almost always the browser serving a stale cached `index.html` — always tell the user to hard-refresh (Ctrl+Shift+R) and confirm the source shows the new `?v=`.
...
**⚠ Ship / git state (READ THIS before touching Slip Studio — updated 2026-07-06):**
- **`main` = web v140 = LIVE** at madderverse.org/slip-studio/ (`index.html` reads `?v=140`). The v137–140 batch (Lane-1 content, stamps removed, set-handle fix, doc updates) was **meant to be HELD off main** but got **accidentally pushed to `origin/main`** by a concurrent Pootery session (bare `git push origin HEAD:main` swept up the two Slip commits already layered on local main) and auto-deployed. Onion was asked and **chose to leave it live** — no revert/git surgery. **The old "held off main" plan is DEAD; `main` is now the base.**
- History is **fully linear**: `origin/main` is strictly ahead of `origin/slip-next-batch` (by one Pootery commit); `slip-next-batch` has **nothing** main lacks. **`slip-next-batch` is retired — do NOT plan a `slip-next-batch → main` merge** (already there; would only risk double-apply confusion). Continue remaining Slip work **from `main`**, isolated in a **git worktree** (a concurrent Pootery session shares this OneDrive checkout — that's what shipped this batch early). Current worktree: `C:\Users\kelly\slip-next-worktree`, branch `slip-next` (upstream unset so a bare push can't auto-deploy); deploy via explicit `git push origin HEAD:main` only when Onion says.
- **Android/version:** live web is now **v140** but Play still has **vc13 = v136 in review** → web/Play are intentionally **MISMATCHED** now (accepted; the matching the hold protected is already gone). Next AAB = **vc14 bundling v140** (bump versionCode 13→14 / versionName).
...
**Android build state (current — supersedes the "Android build state" paragraph above).** Play AAB = **vc13 / 2.4.0 / web v136, uploaded to Play 2026-07-05, awaiting review** (first Play build since vc7/2.2.1; carries the whole session's work). Recipe unchanged: JDK 21 `bundleRelease` with the `-PSLIP_*` signing flags (`assembleRelease` for sideload APKs). **www sync now ALSO copies `assets/frescoes`** (band friezes) alongside motifs/patterns/frames/img (+ index.html/main.js/style.css/vendor + preload-only backgrounds). **Gotcha:** `slip-studio/assets/motifs/mythological-creatures/` is an UNWIRED, untracked, unoptimized (~5.6M) WIP motif pack — EXCLUDE it from the bundle (not in `MOTIF_PACKS`); wire + optimize before shipping. Asset optimizer = Pillow (`scratchpad/optimize_assets.py`: 640px q82 JPEG, 256-colour FASTOCTREE PNG preserving alpha).
