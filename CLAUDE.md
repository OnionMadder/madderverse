...
**Done (no longer TODO):**
- **App icon + splash** — Slip Studio rose-striped vase icon is in place at `slip-studio-app/assets/icon.png` (1080×1080) and has been propagated to `android/app/src/main/res/mipmap-*/ic_launcher{,_round,_foreground}.png` via `npx @capacitor/assets generate --android`. Don't regenerate unless replacing the source.
...
**Cache-busting.** `index.html` loads `main.js?v=N` + `style.css?v=N`; **bump `N` on every web change** (currently **v170**, all live on `main` — see the "Gallery + decorate polish run" note at the very bottom for everything since v140). The recurring "my change reverted / does nothing" reports were almost always the browser serving a stale cached `index.html` — always tell the user to hard-refresh (Ctrl+Shift+R) and confirm the source shows the new `?v=`. When iterating in the preview, the page URL's `&t=` does NOT bust the module — you must bump `?v=N` (or the cached `main.js` serves and `window.__slip` looks stale/missing).
...
- **State:** `state.dips[]` (ordered, non-destructive) + `dipPreview` (live drag) → `renderDips()` replays them (`paintDip`/`paintDrips`/`paintPresetDip`). `dipPreview`'s `let` MUST sit before `init()` — `currentLook()` reads it (TDZ bug if declared late).
...
**⚠ Ship / git state (READ THIS before touching Slip Studio — updated 2026-07-07; worktree addendum 2026-07-24):**
- **Worktree cleanup 2026-07-24:** a rats-nest of ~11 nested/orphaned git worktrees under the OneDrive checkout was removed. The clean outside-OneDrive worktree `C:\Users\kelly\slip-next-worktree` is **now checked out on `main`** (was `slip-next`) — treat it as the working copy; the in-place OneDrive checkout (`…/website backups/madderverse`) may still sit on the **stale, abandoned `slip-next-batch`** (a dead Pootery-v64-era branch — do NOT use it). OneDrive file-locking on the in-place checkout actively breaks git ops (a `git checkout main` there fails on locked files), which is why work belongs in the outside worktree. Beyond that, the v170 state below is current.
- **`main` = web v170 = LIVE + DEPLOYED** at madderverse.org/slip-studio/ (`index.html` reads `?v=170`). Two large runs (2026-07-06 v141→v149, 2026-07-07 v150→v168) were built on the `slip-next` worktree and pushed to `main`; all live. **2026-07-08:** **v169** = pot+lid set gallery-card fix (mobile) — the set tile used `members[0]`, but members are ts-sorted so the later-saved lid fronted the tile with its own name/glaze (now picks the pot half); and the assembly thumb (shot with the pulled-back `CAM_ASSEMBLED_*` camera) sank into the backdrop when staged with the solo `POT_FOOT_FRAC` (now staged with a computed `ASSEMBLY_FOOT_FRAC`). **v170** = gallery reduced to a **single Studio backdrop** — Forest/Gallery backdrops + the picker removed (the v163 "player-choosable backdrops" is thus REVERTED), `forest.jpg`/`gallery.jpg` deleted (~92KB); `state.galleryBg` is always `showcase`. See the dated notes at the very bottom for the earlier feature list.
- **`slip-next-batch` (the older branch) is retired.** Continue Slip work **from `main`**, isolated in the **git worktree** (a concurrent Pootery session shares this OneDrive checkout — that cross-contamination is real; it swept commits to main once and a Pootery push diverged mid-run, both handled by rebasing). Current worktree: `C:\Users\kelly\slip-next-worktree`, branch `slip-next`. **Always `git fetch` before pushing** and deploy via an explicit `git push origin slip-next:main`; if `origin/main` moved (Pootery), `git rebase origin/main` first (their commits never touch `slip-studio/`, so it's always clean).
- **Deploy loop:** commit → `git push origin slip-next:main` → `gh run watch <deploy-run>` → confirm live `?v=`. Verify Slip changes in the preview via `window.__slip` + `preview_eval` (WebGL screenshots time out; the landing/shape-picker SVG icons DO screenshot). Serve the worktree on port 8798 (the `slip-wt` launch config).
- **Android/version:** **vc15 / 2.5.1 (web v170) is BUILT & signed** (2026-07-08) — `slip-studio-app/android/app/build/outputs/bundle/release/app-release.aab`, copied to `Desktop/slip-studio-v2.5.1-vc15.aab` (19.5M) — **staged for Onion to upload to Play.** Supersedes the earlier `Desktop/slip-studio-v2.5.0-vc14.aab` (v168), which predated the v169/v170 set-gallery + single-backdrop fixes — **don't upload vc14.** Play track is currently vc13 (v136); this is the next upload. (upload + release notes + store event are Play Console, user-side; copy drafted in `slip-studio/PLAY_STORE_LISTING.md`). **Rebuild recipe:** bump `versionCode`/`versionName` in `slip-studio-app/android/app/build.gradle`, re-sync www (below), `npx cap copy android` from `slip-studio-app/`, then JDK 21 `bundleRelease` with the `-PSLIP_*` signing flags. **www-sync copies these into `slip-studio-app/www/assets/`** (rebuild `www/assets` fresh to avoid drift): `audio`, `sfx`, `favi`, `bands`, `frames`, `img`, `motifs` (incl. the wired+optimized `mythological-creatures`, 712K), `patterns` (incl. `enamels/`, `frescoes/`), `backgrounds/gallery` (now just the single `showcase.jpg` Studio backdrop) **+ preload-only `backgrounds/preload`** — NOT the big Art/Botanical/Digital/Paper/Motion backdrop folders (those download on demand). Plus `index.html`/`main.js`/`style.css`/`vendor`.
...
- **Content generosity:** `DIP_SET_PACKS` (Sky/Sea/Ember/Garden/Earth, 30 gradients), `GLAZE_PACKS` grown, `DECO_COLOR_PACKS` (Basics/Warm/Cool/Bright), the **Mythical** motif pack (`mythological-creatures`, optimized + wired). **Every PACKS table + `GALLERY_BACKDROPS` must be defined ABOVE the `state` object** — `state` reads them at init, and a const referenced before definition is a TDZ ReferenceError that kills module init (silent: `window.__slip` just never appears).
...
- **Specials** (`launchCookie` roll, gated on `TUNE.allowFrenzy`): **golden** cookie (5× points); **frenzy** cookie → `startFrenzy()` (~5s cookie storm: rapid spawns, no bombs, 2× points, rainbow blade, code-rain races). Reusable pattern for future power-ups.
...
- **Feast** finale: Tub eats the whole pile in exactly **3 bites** (chomp-chomp-swallow, `chompPile`) regardless of size, then a burp.
...
## Petalcraft — cozy flower-breeding game (LIVE, advertised)

Lives at `petalcraft/` (flat shape: `index.html` + `game.js` + `style.css`, plus `DESIGN.md` = full spec). **LIVE + advertised** at madderverse.org/petalcraft/ (hub grid card #8, in sitemap/llms/JSON-LD). Vanilla JS + CSS + WebAudio + SVG, **zero art assets** (CSS/SVG-drawn flowers). Mendelian genetics reimplemented from the community-documented AC:NH flower model (phenotype tables cross-verified against Joey Parrish's GPLv3 `phenotypes.py` — see the code comments and `DESIGN.md §10` for attribution); genotype is hidden ("accidental discovery"). No timers, no fail states, no ads, no accounts, no IAP — the Madderverse Promise. Real-time accelerated clock with offline catch-up; localStorage save (`petalcraft-save`, schema-versioned with a `migrateSave` step — currently **v3**).

- **Cache-bust:** `index.html` loads `game.js?v=N` + `style.css?v=N` — **bump `N` on every web change** (currently **v6**). Stale-cache "my change did nothing" reports = hard-refresh (Ctrl+Shift+R) and confirm the new `?v=`.
- **Species (unlock chain):** cosmos (free) → tulips (first cosmos hybrid) → pansies (5 cosmos dex slots) → hyacinths (10 total discoveries). Each has a phenotype `table`, a `dex` colour list, and a `rare` list (signature endgame flowers) in the `SPECIES` object.
- **Phase 3 systems (2026-07-24):** rare-tier collectibles (bigger sparkle + jingle + keepsake card + a `★ Rares` trophy shelf), a card-list Bloombook with All/Found/Rare filters + inline flavor + locked-species tabs, and progression-unlocked **garden ornaments** (cozy scene decorations, no gameplay effect). Display name is variablized as `GAME_NAME` in `game.js` (drives topbar + `document.title` + toasts); a rename still needs the static `<title>`/meta in `index.html` edited too (Onion is considering a rename — folder is still `petalcraft/`).
- Dev handle: `window.__petalcraft` (state, `advanceDay`, `unlockAll`, `reset`, rarity helpers). Phase 6 (Android paid app via Capacitor, pkg `org.madderverse.petalcraft`) is not built yet — see `DESIGN.md §7`.
