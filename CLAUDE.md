...
- **Next Play upload is versionCode 4, versionName "2.0"** (the local Capacitor wrap is now at vc4/vn2.0; vc3 was uploaded but exceeded Play's "significantly increases download size" warning threshold because the on-demand bg folders were bundled by mistake — discard the vc3 draft and upload vc4). v1.1 (vc2) is the previously-live build. Bump these in `slip-studio-app/android/app/build.gradle` for every subsequent upload.
...
**Cache-busting.** `index.html` loads `main.js?v=N` + `style.css?v=N`; **bump `N` on every web change** (currently **v98**). The recurring "my change reverted / does nothing" reports were almost always the browser serving a stale cached `index.html` — always tell the user to hard-refresh (Ctrl+Shift+R) and confirm the source shows the new `?v=`.
