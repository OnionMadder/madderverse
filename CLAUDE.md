...
- **`main` = web v149 = LIVE + DEPLOYED** at madderverse.org/slip-studio/ (`index.html` reads `?v=149`). A large 2026-07-06 batch — glaze chemistry (dip reactions, speckle/crackle Stoneware pack), Glossy/Matte/Lustre finishes, slip-trailing + wax-resist tools, kiln-shelf photo export + 2 photo styles, 5 new shapes incl. auto-handle mug, the **Mythical** motif pack, and dead-stamp-code removal — was built on the `slip-next` worktree and pushed `slip-next → origin/main` (Pages deploy verified).
- **`slip-next-batch` (the older branch) is retired.** Continue Slip work **from `main`**, isolated in a **git worktree** (a concurrent Pootery session shares this OneDrive checkout). Current worktree: `C:\Users\kelly\slip-next-worktree`, branch `slip-next` (== origin/main after the push). Deploy via an explicit `git push origin slip-next:main` (or `HEAD:main`) when Onion says.
- **Android/version:** web is now **v149** but Play is still **vc13 (v136)** and the AAB is deliberately **HELD** (Onion's call) — web is well ahead of Play. Next AAB = **vc14** (bump versionCode 13→14 / versionName); www-sync must copy through v149 **incl. `assets/motifs/mythological-creatures`** (now optimized + wired) alongside frescoes/motifs/patterns/frames/img.
