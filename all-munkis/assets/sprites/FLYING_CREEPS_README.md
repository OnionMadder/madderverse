...
JSON (Hash). The STANDARD sheet is **6 rows (creeps) × 5 columns
(frames)** — currently a **1092×1577** image. Rows top→bottom are the
6 creeps (blue, green, orange, purple, red, yellow); columns left→right
are frames 1–5. Frame rects are **measured tight alpha bounding boxes**
(NOT a uniform grid — the art is hand-arranged, sprites vary in size),
+6px pad, named **`creep-<colour>-<n>`**; the loader groups by
`<colour>` and orders each creep's frames by `<n>`:

```json
{
  "frames": {
    "creep-blue-1": { "frame": { "x": 12, "y": 8,   "w": 224, "h": 337 } },
    "...":          "(creep-blue-2..5, then creep-green-1..5, etc.)",
    "creep-yellow-5": { "frame": { "x": 880, "y": 1430, "w": 224, "h": 230 } }
  },
  "meta": {
...
`meta.refMax` is the **single global scale reference** (max frame
dimension across ALL creeps): the renderer scales every creep by
`box / refMax` and centres each frame, so **all creeps render at the
same basic size and perfectly centred regardless of per-frame
wing-width variation**. The sprites are drawn flying **left→right**;
the engine mirrors (`scaleX(-1)`) any creep travelling leftward so it
always faces its direction of flight.

`flying-creeps.json` is **generated by the alpha-bbox detector** (it
measures the real sprite boxes + the global refMax) — regenerate it
with that detector whenever the art changes; never hand-edit the rects.
