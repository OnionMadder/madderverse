...
## ⚠ Ship state & git workflow — READ FIRST (updated 2026-07-07)
- **`main` = web v149 = LIVE + DEPLOYED** at madderverse.org/slip-studio/. This batch (Phases A/B/C/D/F + the Mythical motif pack) was built on the `slip-next` worktree and pushed `slip-next → origin/main` on 2026-07-06; Pages deploy verified. Work continues **from `main`** (the worktree `C:\Users\kelly\slip-next-worktree` is now == origin/main).
- **Isolation:** a concurrent Pootery session shares the OneDrive checkout — keep Slip work in the **git worktree** off `origin/main`; deploy with an explicit `git push origin slip-next:main` (or `HEAD:main`) when Onion says.
- **Android/version:** web is now **v149**; Play is still **vc13 (v136)** and the AAB is deliberately **HELD** — web is well ahead of Play (Onion's call). Next AAB = **vc14** (bump versionCode 13→14 / versionName); www-sync must copy through v149 **incl. `assets/motifs/mythological-creatures`**. See CLAUDE.md + the `project_slip_studio_android_build` memory.

## ✅ Progress (2026-07-06 build session, on branch `slip-next` in worktree `C:\Users\kelly\slip-next-worktree`, cache now **v148**, NOT yet deployed)
Done + verified + **DEPLOYED (live v149)**: **A1** dip colour reactions · **A2** speckle/crackle glazes (Stoneware pack) · **A3** Glossy/Matte/Lustre finish selector · **B1** slip trailing (raised bead) · **B2** wax resist (fires bare clay) · **C1** kiln-shelf photo export · **C3** two more photo styles (Spotlight/Pastel) · **D1** five new shapes + auto-handle mug · **E** wired the **Mythical** motif pack (art optimized 6.0MB→697KB) · **F** dead-stamp-code removal.
**Still open:** B3 marbling (stretch), C2 named collections, D2 foot/rim, D3 fluting, more Lane-2 packs. Visual-polish items to eyeball on-device before the AAB: A2 speckle density, A3 lustre look, B1 bead relief at fired scale, B2 bare-clay patches, C1 shelf layout, the Mythical motifs.
