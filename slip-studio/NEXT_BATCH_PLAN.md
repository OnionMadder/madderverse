...
Done + verified via `?dev __slip`: **A1** dip colour reactions · **A2** speckle/crackle glazes (Stoneware pack) · **A3** Glossy/Matte/Lustre finish selector · **B1** slip trailing (raised bead) · **B2** wax resist (fires bare clay) · **C1** kiln-shelf photo export · **C3** two more photo styles (Spotlight/Pastel) · **D1** five new shapes + auto-handle mug · **F** dead-stamp-code removal.
**Still open:** B3 marbling (stretch), C2 named collections, D2 foot/rim, D3 fluting, **E** wire the `mythological-creatures` pack (needs the art optimized + present — Lane 2). Visual-polish items to eyeball on-device before the AAB: A2 speckle density, A3 lustre look, B1 bead relief at fired scale, B2 bare-clay patches, C1 shelf layout.
