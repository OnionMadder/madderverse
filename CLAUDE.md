...
**⚠ Ship / git state (READ THIS before touching Slip Studio — updated 2026-07-25):**
- **`main` = web v182 = LIVE + DEPLOYED** at madderverse.org/slip-studio/ (`index.html` reads `?v=182`). The **sculpting overhaul (2026-07-25, v171→v180)** and **Display mode (2026-07-25, v181→v182)** landed and are live — see the notes at the very bottom. Everything through v170 (dated notes below) is also live.
...
- **Worktree / git:** the v171→v180 run was built + deployed from an **in-OneDrive git worktree** at `…/madderverse/.claude/worktrees/slip-studio-exploration-524fee` (branch `claude/slip-studio-exploration-524fee`), and `git fetch`/`rebase`/`push` all worked fine there (the earlier "OneDrive locking breaks git ops" warning did NOT bite this worktree path — the clean outside worktree `C:\Users\kelly\slip-next-worktree` is still an option if it does). The old `slip-next` / `slip-next-batch` branches are retired — continue Slip work **from `main`**, isolated in a **git worktree**.
...
- **Cache-bust:** `index.html` loads `game.js?v=N` + `style.css?v=N` — **bump `N` on every web change** (currently **v12**). Stale-cache "my change did nothing" reports = hard-refresh (Ctrl+Shift+R) and confirm the new `?v=`.
- **⚠ Cosmos = colour-mixing starter (redesigned 2026-07-25; DIVERGES from the other species).** Cosmos's 3 genes are pigment presence R/Y/B ("present" at strength ≥1); colour = which pigments show (R=red, Y=yellow, B=blue, R+Y=orange, Y+B=green, R+B=purple, none=white, all=black). Seeds are the 3 **primaries** (red `200`, yellow `020`, blue `002`); crossing two primaries **deterministically** yields the mix (red×yellow→orange, etc.), so breeding teaches colour theory. dex = red/orange/yellow/green/blue/purple (6); **white + black are latent RARES** (deep/accidental crosses) shown on the `★ Rares` shelf. `isHybridColor` is now **species-aware** (`seedColorsFor`). **The other 7 species still use the AC:NH palettes** (white/pink/orange/black etc.) — unifying them onto the colour-mixing model is a candidate future task.
- **Art pipeline is LIVE (torn-paper collage), cosmos is the first shipped species.** Inputs (untracked, local working art): raw paper shapes `florigami/shapes/*.png` (circle/oval/kite/seed/leaf/stem/triangle/rhombus/pentagon), rawpixel patterns `florigami/patterns/*.png`. Tools: **petal-formation editor** `florigami/tools/petal-editor.html` (design a flower layout by rings + per-petal drag → export formation JSON; wide shapes auto-align long-axis outward, "petal spin" + "orient all" controls); **compositor** `florigami/tools/compositor.py` (stamps a sprite sheet from a formation + shapes + a pattern). Formations saved in `florigami/formations/`. **Shipped:** `florigami/assets/img/flowers/cosmos.png` (8 colours × seed/sprout/bud/bloom, "dots A" pattern). The in-game sprite system (`SPECIES[x].sprites`, `initSpriteSheets`, `spriteFrameFor`, `applySprite`; sheet rows = **dex + rares** via `spriteColorsFor`) draws from the sheet when it loads, **CSS shapes stay the fallback**. Locked recipe: patterned petals toned to the flower colour, **watercolour pigment-pool shading** (edge emerges from the shading, no drawn line), **plain watercolour centre**. Grid convention in `ASSETS.md`; Gemini prompts in `PAPER_ART_PROMPTS.md`.
...
- **Species (unlock chain, total-discovery gates):** cosmos (free) → tulips (first cosmos hybrid) → pansies (5 cosmos dex slots) → hyacinths (10 total) → lilies (14) → mums (18) → windflowers (23) → roses (28, 4-gene endgame w/ blue rose). Each has a phenotype `table`, a `dex` colour list, and a `rare` list in the `SPECIES` object.
