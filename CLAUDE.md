...
**Next upload: `Desktop/slip-studio-v2.8.0-vc23.aab`** (18.8 MB, built 2026-08-04) — the replayability release, web **v225 → v232**: the test-tile wall, 72 glaze reactions (up from 21), wood/soda/raku firing types, re-firing a finished pot, and kiln loads. Verified inside the bundle: `base/assets/public/index.html` reads `?v=232`, the bundled `main.js` is byte-identical to the shipped web v232, only the `gallery` (showcase.jpg alone) + `preload` backdrop folders are packaged, and `jarsigner` confirms `CN=Slip Studio, OU=Mad Sundar LLC` with SHA-256 `53:53:1C:8E…` — the same upload key. Release notes = the v2.8.0 block in `slip-studio/PLAY_STORE_LISTING.md` (498 chars). ⚠ The **live listing description is still the v2.2-era copy** (see below) — worth fixing in the same visit. The superseded `slip-studio-v2.7.0-vc22.aab` was deleted 2026-08-04 once vc23 was verified, so `Desktop/` again holds exactly one Slip Studio AAB and there's no wrong-file risk. (vc22 itself is safe — Play holds the uploaded copy, and it's reproducible from commit `14eb598` via the rebuild recipe.)
...
**Still stale on the live listing:** the Aug 1 submission changed only the app name and the video. The **full description is still the v2.2-era copy**, which advertises "ten stamp shapes" — stamps were removed in web v138 and the dead code deleted in v146, so the shipping app does not have them. Rewritten v224-accurate copy is ready in `slip-studio/PLAY_STORE_LISTING.md` (commit c2a93a5) and needs pasting into Play Console; the staged screenshots in `slip-studio-app/assets/` are July 5–8 and also predate the sculpting overhaul, the gallery rework and Display mode.

*(Historical: Play AAB was vc13 / 2.4.0 / web v136, uploaded 2026-07-05, and stayed there for nearly a month while web ran ahead to v224.)* Web is now far ahead at **v182** (see the ship-state block above for the current www-sync folder list). Recipe unchanged: JDK 21 `bundleRelease` with the `-PSLIP_*` signing flags (`assembleRelease` for sideload APKs). The old `mythological-creatures`-is-unwired gotcha is **RESOLVED** — it's optimized + wired into `MOTIF_PACKS` now. Asset optimizer = Pillow (throwaway scripts written to the session scratchpad, not committed): 640–800px q82 progressive JPEG for opaque tiles; downscale + 256-colour FASTOCTREE PNG **preserving alpha** for transparent art (motifs/patterns/frescoes) — typically 6–15MB → sub-1MB with no visible loss.
