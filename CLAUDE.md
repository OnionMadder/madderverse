...
**⚠ Ship / git state (READ THIS before touching Slip Studio — updated 2026-07-07):**
...
- **Android/version:** **vc14 / 2.5.0 was BUILT & signed against web v168** (2026-07-07) — `slip-studio-app/android/app/build/outputs/bundle/release/app-release.aab`, copied to `Desktop/slip-studio-v2.5.0-vc14.aab` (19M). ⚠ **That vc14 AAB predates the v169/v170 web fixes — it still has the set-gallery bug, so DON'T upload it as-is.** The next Play AAB should be **vc15 built off web v170** (bump versionCode 14→15 / versionName, re-sync www, `bundleRelease`). Play track is currently vc13 (v136). (upload + release notes + the "Major update" store event are Play Console, user-side; copy drafted in `slip-studio/PLAY_STORE_LISTING.md`). **www-sync now needs these asset folders** copied into `slip-studio-app/www/assets/`: `motifs` (incl. the wired+optimized `mythological-creatures`), `patterns` (incl. `enamels/`, `frescoes/`), `bands` (band friezes — moved from `frescoes/`), `frames`, `img`, `backgrounds/gallery` (now just the single `showcase.jpg` Studio backdrop) **+ preload-only `backgrounds/preload`** — NOT the big Art/Botanical/Digital/Paper/Motion backdrop folders.
