...
`Desktop/slip-studio-v2.8.0-vc26.aab` (18.0 MB) is the one to upload. Verified: bundled `index.html` reads `?v=245`, bundled `main.js` byte-identical to the shipped web v245, all 82 referenced assets present, only `gallery` (showcase.jpg alone) + `preload` backdrop folders packaged, manifest reads versionCode 26 / versionName 2.8.0 / `org.madderverse.slipstudio`, and the cert is `CN=Slip Studio, OU=Mad Sundar LLC` SHA-256 `53:53:1C:8E…` — the same upload key as vc22. It covers web **v225 → v245**: the whole replayability run (firing types, re-fire, kiln loads, test-tile wall, 72 reactions) plus the noticeboard, study shelf, pot sharing and showroom.
