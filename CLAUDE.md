...
**⚠ THE HOLD IS LIFTED — vc26 is BUILT, SIGNED and STAGED for upload (2026-09-01).** Onion asked to publish; the Aug-4 hold ("nothing goes to Play until the UI polish and bug hunt are finished") is satisfied and over.

`Desktop/slip-studio-v2.8.0-vc27.aab` (18.0 MB) is the one to upload. (vc26 was built first, then superseded and deleted the same day when a real phone render found three layout bugs — see below.) Verified: bundled `index.html` reads `?v=248`, bundled `main.js` and `style.css` byte-identical to the shipped web v248, all 82 referenced assets present, only `gallery` (showcase.jpg alone) + `preload` backdrop folders packaged, manifest reads versionCode 27 / versionName 2.8.0 / `org.madderverse.slipstudio`, and the cert is `CN=Slip Studio, OU=Mad Sundar LLC` SHA-256 `53:53:1C:8E…` — the same upload key as vc22. It covers web **v225 → v248**: the whole replayability run (firing types, re-fire, kiln loads, test-tile wall, 72 reactions) plus the noticeboard, study shelf, pot sharing and showroom.

**The launcher icon was refreshed in this build** — the long-standing drift is fixed. `slip-studio-app/assets/icon.png` is now a 1080² Lanczos upscale of the Jul-7 listing art (the rainbow-swirl lidded jar); the old May-27 rose-striped-vase source is kept at `assets/icon-OLD-may27.png.bak`. Mipmaps regenerated with `npx @capacitor/assets generate --android` and confirmed to carry the new art. The 512 source is ample: the largest launcher output is 432² (adaptive foreground).

**Store screenshots are DONE and staged** at `Desktop/slip-studio-screenshots/` — eight 1080x1920 PNGs, numbered in upload order (finished pot, showroom, shaping, glazing, study shelf, noticeboard, decorating, glaze wall). They were captured from the real app by driving headless Chrome over the DevTools Protocol (`scratchpad/cdp.mjs` + `full.mjs`; node 22 has a global WebSocket, so no puppeteer). **Two things make that repeatable:** WebGL needs `--enable-unsafe-swiftshader --use-angle=swiftshader`, and the viewport must be emulated at **360x640 CSS with deviceScaleFactor 3** — that renders the true phone layout and lands on exactly 1080x1920. Emulating a 1080-wide CSS viewport instead gives a tablet layout at the right pixel size, which is wrong and was the first attempt.

**⚠ Rendering the app at phone metrics found three bugs that assertions had missed** (all fixed in v248, all in vc27): the study meter's empty pill painted on every screen because `.study-meter`'s `display` outranked the `[hidden]` UA rule — and `el.hidden` read `true` the whole time, so only a real render caught it; the showroom fitted two pots to a phone shelf; and the study-shelf blurb was squeezed into a ~150px column. **The lesson is the same one the touch-target trap taught: check what is painted, not what the DOM says.**

**Play Console work is user-side and still outstanding:** upload the AAB, paste the new full description and the vc26 "What's new" (both in `slip-studio/PLAY_STORE_LISTING.md`, both sized with CRLF headroom), and consider refreshing the screenshots — the staged ones are July 5–8 and predate the sculpting overhaul, gallery rework, Display mode, and everything in v225–v245. (Same posture as the standing "defer itch/Play" rule, but stricter and for a specific reason: vc13 sat broken on the store for a month and cost refunds, so she is not shipping another build sight-unseen.)

**Sideload test APK: `Desktop/slip-studio-v2.8.0-vc25-TEST.apk`** (19.3 MB, built 2026-08-25, **web v241**) — for Onion to bug-hunt on real hardware; NOT for Play. Supersedes the vc24 TEST apk (deleted). Cut after the 2026-08-25 bug hunt over the replayability-era diff (v226→v240), which found and fixed five confirmed bugs, all shipped as **v241**: (1) Save on an unchanged lone pot duplicated the entry (guard was set-only); (2) a loaded set's partner lost its firing — `loadAsCapturedState` omitted `kiln`/`firingType` (+`heightScale`/`lidBaseRatio`), so a wood set reloaded with one clean half and a swap-then-save stripped the firing for good; (3) the firing preference leaked — viewing a raku pot made the next NEW pot fire raku while the chips said Electric (`resetPot` now restores from `slip-firing`; `loadPot` re-syncs the chips); (4) `lidBaseRatio` was never persisted, so the v238 Match-rim overhang fix evaporated on reload and could borrow a stale ratio from another lid (now in `corePieceFields` + restored in `loadPot`); (5) the kiln-load picker represented a set by whichever member saved FIRST, so a set saved lid-active fired (and copied) a standalone lid — it now prefers the pot member. Verified in-preview at runtime (double-save → 1 entry; partner kiln survives reload; ratio round-trips at 1.0739; pref restores; pick list offers the pot). Everything else in that diff checked clean: reactions tables, tile-vs-pot render agreement, boot with all `slip-*` keys corrupted, focus traps + Escape ordering, modal z-order, pointer-events, reduced motion. Verified in the APK: bundled `index.html` reads `?v=241`, bundled `main.js` byte-identical to shipped web v241, only `gallery`+`preload` backdrops packaged. **Verify APK signing with `apksigner`, not `jarsigner`** — Gradle signs APKs with **v2 scheme only**, so `jarsigner -verify` reports "jar is unsigned" and that is a false alarm, not a broken build. `$ANDROID_HOME/build-tools/37.0.0/apksigner.bat verify --print-certs` confirms v2, `CN=Slip Studio, OU=Mad Sundar LLC`, SHA-256 `53531c8e…` — the same key. ⚠ **The launcher icon in this APK is still the old May 27 art** (see above); it's a functional test build, and the icon doesn't affect what's being tested. Gradle is now at **versionCode 24**, so the eventual Play build is vc24+ (`slip-studio-app/` is outside git, so that bump isn't in any commit).

**(Historical — superseded by vc26 above.) The vc23 AAB was STALE and is gone from the Desktop.** `Desktop/slip-studio-v2.8.0-vc23.aab` (18.8 MB, built 2026-08-04) was cut at **web v232** and web is now at **v238**. It is missing five things, four of them bug fixes found *after* it was built: the lid dead-end (v233), firing-type titles (v234), the bare-lid fix (v235), the lid-fitting fixes (v238), and the launcher-icon refresh above. It was verified sound at the time — bundled `index.html` reads `?v=232`, bundled `main.js` byte-identical to shipped web v232, only `gallery` (showcase.jpg alone) + `preload` backdrop folders packaged, `jarsigner` confirms `CN=Slip Studio, OU=Mad Sundar LLC` SHA-256 `53:53:1C:8E…` — but it has been overtaken. **When the hold lifts: bump to vc24, refresh the icon, re-sync www, rebuild.** Release notes = the v2.8.0 block in `slip-studio/PLAY_STORE_LISTING.md` (498 chars); the feature list in it still describes v232, so re-read it against the code before pasting. The superseded `slip-studio-v2.7.0-vc22.aab` was deleted 2026-08-04 once vc23 was verified (vc22 is safe — Play holds the uploaded copy, and it's reproducible from commit `14eb598`).
...
The problem: Slip Studio is a sandbox with no reason to open it a fifth time. The genre's answer is a merchant economy, which we refused — currency gating content already paid for, deadlines turning making into obligation, selling that *consumes* the pot, star ratings replacing the player's own judgment. What a merchant loop actually provides is a reason to make *this* pot, a constraint to work against, and a sense the work goes somewhere; all three are available without a coin. Three specs were written first and live alongside the code: **`slip-studio/TEST_TILE_WALL.md`**, **`KILN_FIRINGS.md`**, **`COMMISSIONS.md`**. Each names the functions it touches and phases so it can be stopped after any cut. **Commissions phases 1+2 SHIPPED as "The noticeboard" (v242, 2026-08-25)** — board (✉ corner button under the gallery button), give-a-pot picker (kiln-pick pattern, set = pot member, an already-given pot isn't offered again), reply modal, `commissionId` on the entry (NOT in `corePieceFields` — it's where the object went, so re-fires/kiln loads don't inherit it), the "This one lives in…" line on the gallery card, and a Letters tab. State = `slip-commissions` localStorage (corruption-safe) + `commissionId` per entry; sequels gate on `after:` (never time-gated). **The 12 request/reply pairs are Claude-drafted placeholders for Onion to rewrite** — plain strings in `REQUESTS` in main.js; phases 3 (grow to 30–40 pairs) and 4 (set requests) remain open.

**The showroom SHIPPED (v245, 2026-08-26)** — spec in `slip-studio/SHOWROOM.md`, the last pillar from the competitive research (strong collection systems reward with **display, not currency** — Master of Pottery's showroom, AC's museum). Gallery bar → Showroom: every piece standing on CSS shelf boards against a warm wall, **oldest at the top so the room fills downward**, sets standing assembled, tap → load + Display mode. No capacity, no empty slots, no captions, no visitor scores. `potCutoutURL` is `stageThumb`'s pot-isolation half extracted (cutout + legacy key-out, no backdrop); each slot clips at `POT_FOOT_FRAC` (`ASSEMBLY_FOOT_FRAC` for sets) so feet land on the board. `openShowroom` unhides BEFORE rendering — the row chunking measures `clientWidth`. No new assets. ⚠ **Verified functionally, NOT visually** — this pane can't composite, so `loading="lazy"` images never load and screenshots time out; check the room's look on real hardware.

**Pot sharing SHIPPED (v244, 2026-08-26)** — spec in `slip-studio/POT_SHARING.md`: serverless, a pot travels as a `.slippot.json` file through the family's own share sheet (Gallery → Share → Send/Open; `shareOrDownloadBlob` grew a mime param). A file beats data-in-the-photo (messaging apps recompress images and kill the payload) and beats a server gallery (kids' rating + no-data-collected — Dreams locks child accounts out of UGC). **Everything crosses the `SHARE_FIELDS` whitelist in BOTH directions**: export can't leak id/ts/collections/commissions/studies (nothing identity-shaped exists in the format), import validates per-field — enums via `_inTable` (`Object.hasOwnProperty`, because bare `TABLE[v]` accepts `__proto__`/`constructor` — this was caught live), canvases must be `data:image/png`, placement/dip strings refuse `:` and `..`, numbers clamped, 8MB cap. Import mints fresh id/ts/setId + `gifted: true`; gallery card shows "This one came from a friend." Sets travel whole (pot+lid, one file). Kiln seed travels, so a wood firing replays identically on the friend's device.

**The study shelf SHIPPED (v243, 2026-08-26)** — spec in `slip-studio/STUDY_SHELF.md`, born from the competitive research: it is Let's Create! Pottery's order/star-meter loop with the judgment removed. Ten classic forms (moon jar, meiping, ginger jar, chawan, tokkuri, amphora, olla, ukhamba, albarello, kylix) on a shelf modal (landing link beside "The wall"); picking one seeds a plain cylinder (`STUDY_SEED`) with a translucent ghost lathe of the target beside `potGroup` (NOT inside it — height drift must visibly diverge) and a closeness pill. **The meter is information, nothing reads it**: closeness = progress-from-the-starting-lump (`1 − mad/baselineMad`, so every form starts 0% — a fixed spread punished wide bowls), never stored, never shown on the shelf; **any saved study counts as studied** ("a study after Hokusai"). `slip-studies` localStorage + `studyId` on the entry (NOT `corePieceFields`, like `commissionId`); gallery card gains "A study after the …" line; `state.studyId` cleared in resetPot AND loadPot (a re-save of a loaded pot is a copy, not a new study). `updateStudyMeter` runs from `tick()` and is on `__slip` because the preview pane pauses rAF. Blurbs are Claude drafts for Onion to rewrite.
...
**Status: web build advertised on the hub since 2026-08-04; Android vc2 / 1.0.1 SUBMITTED and IN REVIEW on Play as of 2026-08-06** (listing + screenshots done). A 99¢ Pro unlock is built but its billing is inert pending the RevenueCat key; see the game's own CLAUDE.md.

**Status detail:** — grid card (last
real card, before "More Soon"), JSON-LD `ItemList` position 9,
`sitemap.xml`, `llms.txt`. `cover.jpg` exists.

**It is IN REVIEW, not published — don't describe Tiny Canvas as being
on Google Play** (listing copy, social posts, the hub's own About text)
until Onion says it went live. No App Store submission exists at all.
The submitted AAB also bundles `?v=22`, so it predates the raster
coloring pages, the Pro value pass and the 2026-08-06 export fixes —
**deliberately**; Onion is letting it clear review and updating right
after. Don't push to replace the submission, and don't read the
in-review build as the state of the code.
