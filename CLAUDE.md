...
- **Google Play Console (user only):** upload AAB → set Paid + price → listing copy from `slip-studio/PLAY_STORE_LISTING.md` → privacy URL `https://madderverse.org/slip-studio/privacy/` → screenshots + feature graphic (**resize first**) + 512² icon → content rating → submit. First Play upload extracts the signing cert from this keystore (or enroll in Play App Signing).
