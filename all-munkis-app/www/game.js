...
(() => {
    ...
    // undefined and every call below is a silent no-op, so this file
    // stays byte-identical-in-behavior to the web copy when run in a
    // browser. Plugins are loaded by the Capacitor runtime injected
    // into the native WebView — we never import anything here.
    ...
    function setupNativeChrome() {
        ...
        if (StatusBar) {
            // Dark content area; the game is a dark-themed full-bleed
            // canvas so we want a dark status bar that doesn't overlay.
        ...
        if (SplashScreen) {
            // Give the first paint a beat, then fade the splash out.
            setTimeout(() => {
                try { SplashScreen.hide(); } catch (_) {}
            }, 350);
        }
    }
    // Mirror the progress record into native Preferences as a backup of
    // the WebView localStorage. Android WebView localStorage IS durable
    // across app restarts, so this is belt-and-suspenders, not the
    // primary store. Feature-detected; no-op on web. Best-effort only —
    // a failed write never blocks gameplay.
    function mirrorProgressToPrefs() {
        ...
        try {
            const raw = localStorage.getItem(STORAGE_KEY);
            if (raw != null) {
                Preferences.set({ key: STORAGE_KEY, value: raw });
            }
        } catch (_) { /* ignore */ }
    }
    // On native first launch after an OS WebView data clear, localStorage
    // can be empty while Preferences still holds the last good record.
    // Rehydrate before loadProgress() reads it. Returns a promise.
    async function rehydrateProgressFromPrefs() {
        ...
        try {
            ...
            if (cur != null) return; // localStorage already has it
    ...
    async function init() {
        ...
        loadProgress();
        setupNativeChrome();
        // Keep the native Preferences backup in step with every save.
